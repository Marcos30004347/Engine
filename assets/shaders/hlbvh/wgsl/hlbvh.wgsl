...
/// @param i Zero-based sequential leaf number.
/// @returns The flat array index of the leaf node in the node array.
fn leaf_index(i: i32) -> i32 {
    return (uniforms.num - 1) + i;
}

/// @brief Builds the HLBVH hierarchy by setting up leaf and internal nodes from sorted Morton codes.
...
/// @brief Refits AABB bounds bottom-up from leaves to the root using atomic compare-exchange flags.
@compute @workgroup_size(64, 1, 1)
...
/// @brief Returns the length of the common Morton-code prefix between two sorted indices.
/// @param i1 First index into the sorted Morton code array.
/// @param i2 Second index into the sorted Morton code array.
/// @returns The common prefix length, or -1 if either index is out of range.
...
/// @param idx Index of the internal node.
