...
/// @brief Generates a normalized world-space ray direction for a given pixel coordinate.
/// @param pixel_coords The pixel position in screen space.
/// @param resolution The full resolution of the render target (width, height).
/// @returns A normalized vec3 representing the ray direction from the camera through the pixel.
...
/// @brief Tests a ray against an AABB using the slab method and returns the entry distance.
/// @param ray_origin The world-space origin of the ray.
/// @param ray_dir The world-space direction of the ray.
/// @param aabb_min The minimum corner of the AABB.
/// @param aabb_max The maximum corner of the AABB.
/// @returns The entry distance tmin if the ray hits the AABB, or -1.0 if there is no intersection.
...
/// @brief Fast ray-AABB intersection using precomputed inverse direction and origin offset.
/// @param boxmin The minimum corner of the AABB.
/// @param boxmax The maximum corner of the AABB.
/// @param invdir Precomputed per-component inverse of the ray direction.
/// @param oxinvdir Precomputed value of (-ray_origin * invdir).
/// @param t_max The current maximum intersection distance.
/// @returns A vec2 containing (t_entry, t_exit); hit occurs when t_entry <= t_exit.
...
/// @brief Tests a ray against a triangle from the triangle buffer using the Möller–Trumbore algorithm.
/// @param ray_origin The world-space origin of the ray.
/// @param ray_dir The world-space direction of the ray.
/// @param primitiveId The index into the triangles buffer identifying the triangle to test.
/// @param isect Pointer to a RayIntersection to fill with barycentric and depth data on hit.
...
/// @brief Determines whether an HLBVH node is a leaf (stores a single primitive).
/// @param node The node to inspect.
/// @returns True if the node is a leaf (left == right), false if it is an internal node.
fn is_leaf_node(node: HLBVHNode) -> bool {
    return node.left == node.right;
}

/// @brief Traverses the HLBVH and returns the first triangle hit by the ray.
/// @param ray_origin The world-space origin of the ray.
/// @param ray_dir The world-space direction of the ray.
/// @param intersection Pointer to a RayIntersection filled with traversal depth and hit data.
/// @returns True if any triangle was hit, false otherwise.
...
/// @brief Tests a ray against a sphere and returns the nearest positive intersection distance.
/// @param rayOrigin The world-space origin of the ray.
/// @param rayDir The world-space direction of the ray.
/// @param sphereCenter The center of the sphere.
/// @param sphereRadius The radius of the sphere.
/// @returns The nearest positive hit distance, or -1.0 if there is no intersection.
...
/// @brief Shoots one ray per pixel from the camera and writes the traversal-depth shade to the color buffer.
