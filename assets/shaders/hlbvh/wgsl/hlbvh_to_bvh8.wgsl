...
/// @brief Loads the AABB bounds of a BVH2 node as a 2-row matrix (min in row 0, max in row 1).
/// @param nodeId Index of the node to load bounds for.
/// @returns A mat2x3 where row 0 is the AABB minimum and row 1 is the AABB maximum.
...
/// @brief Computes the surface area of a bounding box stored as a mat2x3.
/// @param bounds A mat2x3 where row 0 is the AABB minimum and row 1 is the AABB maximum.
/// @returns The surface area (non-negative) of the given bounding box.
...
/// @brief Initializes the index-pair and signal arrays before BVH8 conversion begins.
...
/// @brief Converts HLBVH binary nodes into BVH8 nodes by collecting up to 8 children per node.
@compute @workgroup_size(64, 1, 1)
