...
@group(0) @binding(2) var srcDepth: texture_depth_2d;
...
/// @brief Loads a single texel from the native depth texture at the given coordinate.
...
/// @returns The depth value at the specified coordinate.
fn spdLoadSource(coord: vec2<u32>, workgroupId: vec3<u32>) -> f32 {
    _ = workgroupId;
    return textureLoad(srcDepth, vec2<i32>(coord), 0);
}
...
/// @brief Compute entry point that runs the SPD depth pyramid reduction using a native depth texture as the source.
@compute @workgroup_size(SPD_WORKGROUP_SIZE_X, SPD_WORKGROUP_SIZE_Y, 1)
fn depth_pyramid_spd_chunked_source_depth(
