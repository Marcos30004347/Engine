...
/// @brief Computes an exclusive prefix sum over each SUBARRAY_SIZE-element block and stores per-block partial sums.
...
/// @brief Adds each workgroup's partial sum to all elements in its corresponding SUBARRAY_SIZE block to produce the final prefix sum.
@compute @workgroup_size(256)
