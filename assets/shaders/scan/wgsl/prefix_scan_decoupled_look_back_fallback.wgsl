...
/// @brief Performs a device-wide exclusive prefix sum using chained scan with decoupled lookback and fallback.
...
/// @brief Initializes the reduction and scan_bump atomic counters to zero before a prefix sum pass.
@compute @workgroup_size(BLOCK_DIM, 1, 1)
fn init(@builtin(global_invocation_id) id: vec3<u32>, @builtin(num_workgroups) griddim: vec3<u32>) {
	/*
	for(var i = id.x; i < info.size; i += 4u * griddim.x * BLOCK_DIM){
		//for(var j = 0u; j < 4u; j++) {
		scan_in[i][0] = 1u;
		scan_out[i][0] = 1u << 31u;
		//}
	...
	*/
	...
	for(var i = id.x; i < info.thread_blocks * 4u; i += griddim.x * BLOCK_DIM) {
		atomicStore(&reduction[i], 0u);
	}

	if(id.x == 0u){
		atomicStore(&scan_bump, 0u);
	}
}
