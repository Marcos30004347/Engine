...
/// @brief Generates a pseudo-random float in [0, 1) from a 2D seed using a sine-based hash.
/// @param seed A 2D vector used as the random seed.
...
/// @brief Generates a deterministic random RGBA color from a float seed.
/// @param seed A float value used to derive the color.
/// @returns A vec4 with randomized R, G, B channels and alpha set to 1.0.
...
/// @brief Extracts a single byte from a 32-bit unsigned integer.
/// @param i The 32-bit value to extract from.
/// @param n The zero-based byte index (0 = least significant byte).
/// @returns The extracted byte value in the range [0, 255].
...
//     return b0 + b1 + b2 + b3;
// }
/// @brief Sign-extends the high bit (bit 7) of each of the four bytes packed in a u32.
/// @param x A u32 containing four packed 8-bit values.
/// @returns A u32 where each byte's bit 7 is replicated across the remaining bits of that byte.
fn sign_extend_s8x4(x: u32) -> u32
{
	return ((x >> 7u) & 0x01010101u) * 0xffu;
}

/// @brief Tests a ray against a single triangle using the Möller–Trumbore algorithm.
...
/// @param isect Pointer to a RayIntersection to fill with hit data if an intersection occurs.
...
/// @brief Computes a 4-replicated octant inversion mask for a ray direction.
/// @param d The ray direction vector.
/// @returns A packed u32 where each byte encodes the axis-sign inversion bits for one of four children.
...
/// @brief Traverses the CWBVH and tests the ray against all intersected triangles.
/// @param origin_tmin Ray origin in XYZ and minimum intersection distance in W.
/// @param dir_tmax Ray direction in XYZ and maximum intersection distance in W.
/// @param hit Pointer to a RayIntersection that receives the final hit color and distance.
...
/// @brief Shoots one ray per pixel from the camera and writes intersection color to the color buffer.
