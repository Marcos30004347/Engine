...
/// @brief Computes the array index of a leaf node given its sequential leaf number.
/// @param i Zero-based leaf number.
/// @returns The flat array index of the leaf node in the HLBVH node array.
fn hlbvh_leaf_index(i: u32) -> u32 {
    return uniforms.targetNumberOfLeaves - 1u + i;
}

/// @brief Initializes the two flag slots for each thread's corresponding node to zero.
...
/// @brief Constructs the HLBVH hierarchy by assigning leaf data and linking internal nodes.
@compute @workgroup_size(64, 1, 1)
...
/// @brief Refits AABB bounds bottom-up from leaves to the root using atomic flags.
@compute @workgroup_size(64, 1, 1)
...
/// @brief Finds the split position within a Morton-code range using binary search on common prefix length.
/// @param span The inclusive [first, last] index range of the current internal node.
/// @returns The index of the split point within the range.
...
//     return left;
// }
/// @brief Counts the number of set bits (complement of leading zeros) in a 32-bit value.
/// @param v The 32-bit unsigned integer to inspect.
/// @returns The count of significant bits (32 minus the leading zero count).
fn clz(v: u32) -> u32 {
    return 32u - countLeadingZeros(v);
}
/// @brief Returns the length of the common Morton-code prefix between two leaf indices.
/// @param i1 First leaf index.
/// @param i2 Second leaf index.
/// @returns The number of matching leading bits, or 0 if either index is out of range.
...
/// @brief Determines the range of leaf indices covered by the internal node at the given index.
/// @param index Index of the internal node.
/// @returns A vec2 containing the inclusive [first, last] leaf index range for this node.
...
/// @brief Computes the surface area of an axis-aligned bounding box.
/// @param aabb The AABB whose surface area is computed.
/// @returns The surface area (non-negative) of the given AABB.
fn get_surface_area(aabb: AABB) -> f32 {
    let d = aabb.maxPoint - aabb.minPoint;
    return max(2.0f * (d.x * d.y + d.x * d.z + d.y * d.z), 0.0f);
}

/// @brief Returns the current number of elements in the max-heap.
...
/// @brief Inserts a key-value pair into the max-heap, maintaining the heap property.
/// @param heap Pointer to the MaxHeap.
/// @param key The node index to store.
/// @param value The priority value (surface area) used to order the heap.
...
/// @brief Removes and returns the key of the element with the highest value from the max-heap.
/// @param heap Pointer to the MaxHeap.
/// @returns The key of the removed maximum element, or U32_MAX if the heap is empty.
...
/// @brief Resets the max-heap to an empty state without deallocating storage.
/// @param heap Pointer to the MaxHeap to clear.
...
/// @brief Initializes flags and signals arrays before CWBVH construction begins.
@compute @workgroup_size(64, 1, 1)
...
/// @brief Converts HLBVH binary nodes into compressed wide BVH8 (CWBVH) nodes with up to 8 children.
@compute @workgroup_size(64, 1, 1)
