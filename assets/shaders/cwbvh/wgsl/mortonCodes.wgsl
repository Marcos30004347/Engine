...
/// @brief Computes a 30-bit Morton code for each AABB centroid and writes it to the output buffer.
...
fn compute_morton_codes(@builtin(global_invocation_id) global_id : vec3<u32>) {
    ...
    var cen : vec3<f32> = (aabbs[idx].maxPoint + aabbs[idx].minPoint) * 0.5;
...
/// @brief Expands a 10-bit integer to 30 bits by inserting two zero bits between every original bit.
/// @param x The 10-bit input value.
/// @returns A 30-bit value with the input bits spread into every third bit position.
...
/// @brief Computes a 30-bit Morton code for a normalized 3D point.
/// @param p A vec3 with components in [0, 1] representing the normalized position.
/// @returns The interleaved 30-bit Morton code encoding the position.
