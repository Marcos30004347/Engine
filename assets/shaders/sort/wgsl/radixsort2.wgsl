...
/// @brief Computes the ceiling of integer division a / b.
...
/// @brief Clears the counts histogram buffer to zero for the current number of workgroups and bins.
...
/// @brief Counts the occurrence of each 4-bit digit in the current pass for each workgroup's block of keys.
...
/// @brief Reduces the per-workgroup histogram counts into a smaller array for subsequent scanning.
@compute @workgroup_size(WG, 1, 1)
fn sort_reduce(
    @builtin(local_invocation_id) local_id: vec3<u32>,
    @builtin(workgroup_id) gid: vec3<u32>,
...
/// @brief Computes an exclusive prefix sum over the reduced histogram bins.
@compute @workgroup_size(WG, 1, 1)
fn sort_scan(
    @builtin(local_invocation_id) local_id: vec3<u32>,
...
/// @brief Adds the reduced prefix sums back to the per-workgroup counts to produce global offsets.
@compute @workgroup_size(WG, 1, 1)
fn sort_scan_add(
    @builtin(local_invocation_id) local_id: vec3<u32>,
    @builtin(workgroup_id) gid: vec3<u32>,
...
/// @brief Linearizes a 2D workgroup ID to a single flat workgroup index.
/// @param wid The 3D workgroup ID.
/// @param num_wgs The total number of workgroups in each dimension.
/// @returns The linear workgroup index.
fn get_workgroup_id(wid: vec3u, num_wgs: vec3u) -> u32 {
    return wid.x + wid.y * num_wgs.x;
}

/// @brief Scatters keys and values to their sorted positions using precomputed global offsets.
@compute @workgroup_size(WG, 1, 1)
fn sort_scatter(
    @builtin(local_invocation_id) local_id: vec3<u32>,
