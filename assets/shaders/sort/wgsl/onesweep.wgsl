...
/// @brief Returns the subgroup (wave) index of a thread within its workgroup.
/// @param gtid The thread's local index within the workgroup.
/// @param waveGetLaneCount The number of lanes per subgroup.
/// @returns The index of the subgroup this thread belongs to.
fn getWaveIndex(gtid: u32, waveGetLaneCount: u32) -> u32 {
    return gtid / waveGetLaneCount;
}

/// @brief Extracts the 8-bit radix digit from a key at the current sort pass shift position.
/// @param key The 32-bit key to extract from.
/// @returns The 8-bit digit value in the range [0, 255].
fn ExtractDigit(key: u32) -> u32 {
    return (key >> cbParallelSort.e_radixShift) & RADIX_MASK; // RADIX_MASK
}

/// @brief Extracts the packed half-radix index from a key for use with 7-bit packing.
/// @param key The 32-bit key.
/// @returns The 7-bit packed index in the range [0, 127].
fn ExtractPackedIndex(key: u32) -> u32 {
    return (key >> (cbParallelSort.e_radixShift + 1u)) & HALF_MASK; // HALF_MASK
}

/// @brief Returns the bit shift (0 or 16) needed to address the correct half of a packed 32-bit histogram entry.
/// @param key The 32-bit key used to determine the shift.
/// @returns 16 if the key's current-pass LSB is set, 0 otherwise.
...
/// @brief Extracts the 16-bit value from a packed 32-bit field based on the key's shift position.
/// @param packed The packed 32-bit value containing two 16-bit fields.
/// @param key The key used to determine which 16-bit half to extract.
/// @returns The 16-bit value from the selected half of the packed field.
fn ExtractPackedValue(packed: u32, key: u32) -> u32 {
    return (packed >> ExtractPackedShift(key)) & 0xffffu;
}

/// @brief Returns the number of keys processed per subgroup in the downsweep for wave sizes >= 16.
/// @param waveGetLaneCount The number of lanes per subgroup.
/// @returns The subpartition size (DS_KEYS_PER_THREAD * waveGetLaneCount).
...
/// @brief Computes the shared-memory offset for a thread in the downsweep for wave sizes >= 16.
/// @param gtid The thread's local workgroup index.
/// @param waveGetLaneCount The number of lanes per subgroup.
/// @param waveGetLaneIndex The lane index of this thread within its subgroup.
/// @returns The shared memory offset for this thread's key slot.
...
/// @brief Computes the global device buffer offset for a thread in the downsweep for wave sizes >= 16.
/// @param gtid The thread's local workgroup index.
/// @param gid The workgroup index.
/// @param waveGetLaneCount The number of lanes per subgroup.
/// @param waveGetLaneIndex The lane index of this thread within its subgroup.
/// @returns The global device offset for this thread's key.
...
/// @brief Returns the number of keys processed per subgroup-group in the downsweep for wave sizes < 16.
/// @param serialIterations The number of serial iterations per subgroup grouping.
/// @param waveGetLaneCount The number of lanes per subgroup.
/// @returns The subpartition size for small-wave downsweep.
...
/// @brief Returns the offset into the global histogram buffer for the current radix shift pass.
/// @returns The base index for this pass in the global histogram.
fn GlobalHistOffset() -> u32 {
    return cbParallelSort.e_radixShift << 5u;
}

/// @brief Returns the total shared-memory histogram size for the downsweep when wave size >= 16.
/// @param waveGetLaneCount The number of lanes per subgroup.
/// @returns The number of u32 histogram entries needed in shared memory.
fn WaveHistsSizeWGE16(waveGetLaneCount: u32) -> u32 {
    return DS_DIM / waveGetLaneCount * RADIX; // DS_DIM / waveGetLaneCount * RADIX
}

/// @brief Returns the total shared-memory histogram size for the downsweep when wave size < 16.
/// @returns The maximum shared-memory size (MAX_DS_SMEM) needed for small-wave downsweep.
fn WaveHistsSizeWLT16() -> u32 {
    return MAX_DS_SMEM; 
}

/// @brief Clears the global histogram buffer to zero before the radix sort begins.
...
/// @brief Computes per-partition and global digit histograms for the current radix sort pass.
...
/// @brief Performs an exclusive prefix sum over the per-partition histograms to produce scatter offsets.
...
/// @brief Scatters keys and payloads from the source buffer to their sorted positions in the destination buffer.
