...
int main()
{
  ...
  std::string vertexShaderSrc = os::io::readRelativeFile("assets/shaders/triangle/spirv/vertex.spirv");
  std::string fragmentShaderSrc = os::io::readRelativeFile("assets/shaders/triangle/spirv/fragment.spirv");
  ...
  graphicsInfo.vertexStage.cullType = CullMode::None;
  graphicsInfo.vertexStage.winding = WindingOrder::CW;
  ...
  depthAttatchment.enabled = true;
  depthAttatchment.comparison = ComparisonOp::ComparisonOp_LessOrEqual;
  ...
  depthStencilAttatchment.enabled = true;
  ...
  while (!window->shouldClose())
  {
    ...
    attatchment.view = rhi->getCurrentSwapChainTextureView(swapChain);
    attatchment.clearValue = Color::rgba(0, 1, 1, 1);
    ...
    renderGraph->enqueueCommandBuffer("DrawTrianglePlass", commandBuffer);
    renderGraph->compile();
