...
public:
  ...
  /*
  void dump(lib::Vector<Value> &out)
  {
    auto currentTable = rootTable.load(std::memory_order_acquire);

    lib::Vector<Value> out;

    while (currentTable)
    {

      for (size_t i = 0; i < currentTable->capacity; i++)
      {
        if (currentTable->entries[i].key.load() != HashTableBucket<Value>::INVALID_KEY)
        {
...
*/
  ...
  /*
    // Get the current value mapped by the key if it exists. If not, the mapping is inserted in the table.
    void getOrInsert(size_t id, Value& val)
    {
      size_t hashedId = hashInteger(id);
      ...
      for (auto table = currentTable; table != nullptr; table = table->prev)
      {
        size_t index = hashedId;

        while (true)
        {
          index &= table->capacity - 1u;

          size_t probedKey = table->entries[index].key.load(std::memory_order_relaxed);
          ...
          val = table->entries[index].value;

          if (table != currentTable)
          {
            insertInTable(currentTable, id, val);
          }
      ...
      while (true)
      {
        ...
        if (newCount < (currentTable->capacity >> 1) + (currentTable->capacity >> 2))
        {
          return insertInTable(currentTable, id, val);
        }
      }
    }
   */
