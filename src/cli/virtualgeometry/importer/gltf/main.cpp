...
using virtualgeometry::VirtualGeometryBuilder;
...
{
  stream
      << "Usage: " << programName << " --input <scene.gltf|scene.glb> --output <scene.virtualgeometry> [options]\n"
      << "\n"
      << "Create a virtual geometry file from a glTF or GLB scene.\n"
      << "\n"
      << "Options:\n"
      ...
      << "\n"
      << "Example:\n"
      << "  " << programName << " --input assets/scene.glb --output build/scene.virtualgeometry \\\n"
      << "    --compression lz4 --pad-pages true\n";
}
...
int run(const Options &options)
{
  ...
  const VirtualGeometryBuildData build = VirtualGeometryBuilder::buildFromGLTFFile(inputPath.string(), options.buildSettings);
