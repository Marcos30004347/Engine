...
namespace jobsystem
{
    ...
    public:
        ...
        static void yield();
    private:
        static inline std::priority_queue<
            DelayedTask,
    ...
    template <typename F, typename... Args>
    auto JobSystem::invoke(F &&f, Args &&...args)
    {
        using Ret = std::invoke_result_t<F, Args...>;
        auto promise = std::make_shared<PromiseHandler<Ret>>();

        fiber::Fiber *fiber = fiber::FiberPool::acquire([=]() mutable
                                          {
    ...
    template <typename T>
    inline T JobSystem::wait(Promise<T> promise)
    {
        return promise->get();
    }
