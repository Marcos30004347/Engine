...
namespace
{
constexpr size_t kSanitizedStackSize = 256 * 1024;
constexpr size_t kDefaultStackSize = 128 * 1024;
constexpr size_t kFallbackMinStackSize = 32 * 1024;
constexpr size_t kWindowsMaxStackSize = 1024ull * 1024ull * 1024ull;
...
size_t getSystemMinStackSize()
{
  ...
  return 4 * 1024;
#endif
#elif defined(MINSIGSTKSZ)
  return static_cast<size_t>(MINSIGSTKSZ);
#else
  return kFallbackMinStackSize;
#endif
}

size_t getSystemMaxStackSize()
{
#if defined(_WIN32)
  return kWindowsMaxStackSize;
#else
  rlimit limit{};
...
size_t getSystemDefaultStackSize()
{
#if defined(_WIN32)
  return kDefaultStackSize;
#elif defined(SIGSTKSZ)
  return static_cast<size_t>(SIGSTKSZ);
#else
  return kDefaultStackSize;
#endif
}
...
size_t Fiber::getMinSize()
{
  ...
  return kSanitizedStackSize;
#else
  return getSystemMinStackSize();
#endif
}

size_t Fiber::getMaxSize()
{
  return getSystemMaxStackSize();
}
...
size_t Fiber::getDefaultSize()
{
  ...
  return kSanitizedStackSize;
#else
  return getSystemDefaultStackSize();
#endif
}
