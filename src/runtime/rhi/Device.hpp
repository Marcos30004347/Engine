...
enum DeviceFeatures
{
  ...
  DeviceFeatures_FragmentStoresAndAtomics = 1 << 19,
};
...
{
  return lhs.format == rhs.format && lhs.loadOp == rhs.loadOp && lhs.storeOp == rhs.storeOp && lhs.blendMode == rhs.blendMode;
}
...
template <> struct hash<rhi::ColorAttatchment>
{
  std::size_t operator()(const rhi::ColorAttatchment &att) const
  {
    ...
    hash_combine(seed, static_cast<uint32_t>(att.blendMode));
    return seed;
  }
};
