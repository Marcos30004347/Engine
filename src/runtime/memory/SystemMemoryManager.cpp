...
void SystemMemoryManager::alignedFree(void *ptr)
{
  aligned_free(ptr);
}
...
{
  (void)sizeof(align);
  lib::memory::SystemMemoryManager::alignedFree(p);
}
...
{
  void *ptr = lib::memory::SystemMemoryManager::alignedAlloc(static_cast<size_t>(align), size);
  if (!ptr)
  {
    throw std::bad_alloc();
  }
