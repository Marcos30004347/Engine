...
// ---------------------------------------------------------------------------
// Opaque resource IDs – pointer-sized values assigned internally by the RHI.
// The Vulkan backend uses object addresses as stable handles for the lifetime
// of the resource.
// ---------------------------------------------------------------------------
enum class BufferId : uintptr_t
{
  Invalid = 0
};
enum class TextureId : uintptr_t
{
  Invalid = 0
};
enum class SamplerId : uintptr_t
{
  Invalid = 0
};
enum class ShaderId : uintptr_t
{
  Invalid = 0
};
enum class BindingsLayoutId : uintptr_t
{
  Invalid = 0
};
enum class BindingGroupsId : uintptr_t
{
  Invalid = 0
};
enum class GraphicsPipelineId : uintptr_t
{
  Invalid = 0
};
enum class ComputePipelineId : uintptr_t
{
  Invalid = 0
};
...
enum class SwapChain : uintptr_t
{
  Invalid = 0
};
...
enum DeviceFeatures
{
  ...
  DeviceFeatures_DrawIndirectCount = 1 << 18,
  DeviceFeatures_GeometryShader = 1 << 19,
  DeviceFeatures_FragmentStoresAndAtomics = 1 << 20,
};
...
enum class AccessPattern
{
  ...
  // Reading and writing data in shaders (storage buffers, storage images)
  // Used when a compute shader both reads and writes to the same resource
  SHADER_READ_WRITE = SHADER_READ | SHADER_WRITE,
...
struct Color
{
  ...
  static Color rgba(float_t R, float_t G, float_t B, float_t A)
  {
    Color color;
    color.R = R;
    color.G = G;
...
{
  ComparisonOp_LessOrEqual,
  ComparisonOp_GreaterOrEqual,
  ComparisonOp_Always,
  ComparisonOp_Equal,
};
...
struct Buffer
{
  ...
  bool operator==(const Buffer &other) const noexcept { return name == other.name; }
  bool operator!=(const Buffer &other) const noexcept { return !(*this == other); }
};
...
struct Texture
{
  ...
  bool isValid() const noexcept { return !name.empty(); }
  bool operator==(const Texture &other) const noexcept { return name == other.name; }
  bool operator!=(const Texture &other) const noexcept { return !(*this == other); }
};
...
struct Sampler
{
  ...
  bool isValid() const noexcept { return id != SamplerId::Invalid; }
  bool operator==(const Sampler &other) const noexcept { return id == other.id; }
  bool operator!=(const Sampler &other) const noexcept { return !(*this == other); }
};

struct Shader
{
  std::string name; // for logging only
  ShaderId id = ShaderId::Invalid;

  bool isValid() const noexcept { return id != ShaderId::Invalid; }
...
{
  std::string name; // for logging only
  BindingsLayoutId id = BindingsLayoutId::Invalid;

  bool isValid() const noexcept { return id != BindingsLayoutId::Invalid; }
  bool operator==(const BindingsLayout &other) const noexcept { return id == other.id; }
  bool operator!=(const BindingsLayout &other) const noexcept { return !(*this == other); }
};

struct GraphicsPipeline
{
  std::string name; // for logging only
  GraphicsPipelineId id = GraphicsPipelineId::Invalid;

  bool isValid() const noexcept { return id != GraphicsPipelineId::Invalid; }
  bool operator==(const GraphicsPipeline &other) const noexcept { return id == other.id; }
  bool operator!=(const GraphicsPipeline &other) const noexcept { return !(*this == other); }
};

struct ComputePipeline
{
  std::string name; // for logging only
  ComputePipelineId id = ComputePipelineId::Invalid;

  bool isValid() const noexcept { return id != ComputePipelineId::Invalid; }
  bool operator==(const ComputePipeline &other) const noexcept { return id == other.id; }
  bool operator!=(const ComputePipeline &other) const noexcept { return !(*this == other); }
};
...
struct BindingGroups
{
  ...
  bool isValid() const noexcept { return !name.empty(); }
  bool operator==(const BindingGroups &other) const noexcept { return name == other.name; }
  bool operator!=(const BindingGroups &other) const noexcept { return !(*this == other); }
};
...
struct BufferView
{
  ...
  {
    return buffer == other.buffer && resourceId == other.resourceId && offset == other.offset && size == other.size && access == other.access;
  }
  bool operator!=(const BufferView &other) const noexcept
  {
    return !(*this == other);
...
struct TextureView
{
  ...
  {
    return texture == other.texture && resourceId == other.resourceId && flags == other.flags && baseMipLevel == other.baseMipLevel && levelCount == other.levelCount &&
           baseArrayLayer == other.baseArrayLayer && layerCount == other.layerCount && access == other.access && layout == other.layout;
  }
  bool operator!=(const TextureView &other) const noexcept
  {
    return !(*this == other);
...
struct ColorAttatchment
{
  ...
  LoadOp loadOp;
  StoreOp storeOp;
  ColorBlendMode blendMode = ColorBlendMode::AlphaBlend;
  ResourceLayout initialLayout = ResourceLayout::COLOR_ATTACHMENT;
  ResourceLayout finalLayout = ResourceLayout::PRESENT_SRC;
};
...
struct DepthAttatchment
{
  ...
  {
    bool enabled = false;
    ComparisonOp comparison = ComparisonOp::ComparisonOp_Always;
  ...
  bool enabled = false;
  Format format = Format::Format_None;
  LoadOp loadOp = LoadOp::LoadOp_Load;
...
// ---------------------------------------------------------------------------
// Hash specializations for ResourceId types (for use in FlatMap / unordered_map)
// ---------------------------------------------------------------------------
template <> struct std::hash<rendering::BufferId>
{
  std::size_t operator()(rendering::BufferId id) const noexcept { return std::hash<uintptr_t>{}(static_cast<uintptr_t>(id)); }
};
template <> struct std::hash<rendering::TextureId>
{
  std::size_t operator()(rendering::TextureId id) const noexcept { return std::hash<uintptr_t>{}(static_cast<uintptr_t>(id)); }
};
template <> struct std::hash<rendering::SamplerId>
{
  std::size_t operator()(rendering::SamplerId id) const noexcept { return std::hash<uintptr_t>{}(static_cast<uintptr_t>(id)); }
};
template <> struct std::hash<rendering::ShaderId>
{
  std::size_t operator()(rendering::ShaderId id) const noexcept { return std::hash<uintptr_t>{}(static_cast<uintptr_t>(id)); }
};
template <> struct std::hash<rendering::BindingsLayoutId>
{
  std::size_t operator()(rendering::BindingsLayoutId id) const noexcept { return std::hash<uintptr_t>{}(static_cast<uintptr_t>(id)); }
};
template <> struct std::hash<rendering::BindingGroupsId>
{
  std::size_t operator()(rendering::BindingGroupsId id) const noexcept { return std::hash<uintptr_t>{}(static_cast<uintptr_t>(id)); }
};
template <> struct std::hash<rendering::GraphicsPipelineId>
{
  std::size_t operator()(rendering::GraphicsPipelineId id) const noexcept { return std::hash<uintptr_t>{}(static_cast<uintptr_t>(id)); }
};
template <> struct std::hash<rendering::ComputePipelineId>
{
  std::size_t operator()(rendering::ComputePipelineId id) const noexcept { return std::hash<uintptr_t>{}(static_cast<uintptr_t>(id)); }
};
template <> struct std::hash<rendering::SwapChain>
{
  std::size_t operator()(rendering::SwapChain id) const noexcept { return std::hash<uintptr_t>{}(static_cast<uintptr_t>(id)); }
};
template <> struct std::hash<rendering::CommandBuffer>
{
  std::size_t operator()(rendering::CommandBuffer id) const noexcept { return std::hash<uintptr_t>{}(static_cast<uintptr_t>(id)); }
};

// ---------------------------------------------------------------------------
// Hash specializations for handle types (identity is now the ResourceId)
// ---------------------------------------------------------------------------
template <> struct std::hash<rendering::Shader>
{
  std::size_t operator()(const rendering::Shader &b) const noexcept { return std::hash<rendering::ShaderId>{}(b.id); }
};

template <> struct std::hash<rendering::Buffer>
{
  std::size_t operator()(const rendering::Buffer &b) const noexcept { return std::hash<std::string>{}(b.name); }
};

template <> struct std::hash<rendering::BindingGroups>
{
  std::size_t operator()(const rendering::BindingGroups &b) const noexcept { return std::hash<std::string>{}(b.name); }
};

template <> struct std::hash<rendering::GraphicsPipeline>
{
  std::size_t operator()(const rendering::GraphicsPipeline &b) const noexcept { return std::hash<rendering::GraphicsPipelineId>{}(b.id); }
};

template <> struct std::hash<rendering::BindingsLayout>
{
  std::size_t operator()(const rendering::BindingsLayout &b) const noexcept { return std::hash<rendering::BindingsLayoutId>{}(b.id); }
};

template <> struct std::hash<rendering::ComputePipeline>
{
  std::size_t operator()(const rendering::ComputePipeline &b) const noexcept { return std::hash<rendering::ComputePipelineId>{}(b.id); }
};

template <> struct std::hash<rendering::Texture>
{
  std::size_t operator()(const rendering::Texture &t) const noexcept { return std::hash<std::string>{}(t.name); }
};

template <> struct std::hash<rendering::Sampler>
{
  std::size_t operator()(const rendering::Sampler &s) const noexcept { return std::hash<rendering::SamplerId>{}(s.id); }
};
...
template <> struct std::hash<rendering::BufferView>
{
  std::size_t operator()(const rendering::BufferView &bv) const noexcept
  {
    ...
    rendering::hash_combine(h, std::hash<rendering::BufferId>{}(bv.resourceId));
    rendering::hash_combine(h, std::hash<uint64_t>{}(bv.offset));
    rendering::hash_combine(h, std::hash<uint64_t>{}(bv.size));
...
inline rendering::ImageAspectFlags operator|(rendering::ImageAspectFlags a, rendering::ImageAspectFlags b)
{
  return static_cast<rendering::ImageAspectFlags>(static_cast<uint64_t>(a) | static_cast<uint64_t>(b));
}

inline rendering::BindingVisibility operator|(rendering::BindingVisibility a, rendering::BindingVisibility b)
{
  return static_cast<rendering::BindingVisibility>(static_cast<uint64_t>(a) | static_cast<uint64_t>(b));
}
